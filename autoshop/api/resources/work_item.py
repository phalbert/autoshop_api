...
class WorkItemResource(Resource):
    ...
    def put(self, work_item_id):
        ...
        try:
            db.session.commit()
            return {'msg': 'work_item updated',
                    'work_item': schema.dump(work_item).data}
    ...
    def post(self):
        ...
        try:
            ...
            if not ServiceRequest.get(uuid=work_item.request_id):
                return ({'msg': 'The supplied service request code doesnt exist'
                         }, 422)
            if WorkItem.get(request_id=work_item.request_id,
                            item=work_item.item):
                return ({'msg': 'The supplied work_item already exists'
                         }, 409)
            else:
                db.session.add(work_item)
                db.session.commit()
                return ({'msg': 'work_item created',
                         'work_item': schema.dump(work_item).data}, 201)
        except Exception as e:
            db.session.rollback()
