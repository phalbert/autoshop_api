...
from autoshop.models import (
    Entity, Job, Employee, ServiceRequest
)
...
class JobSchema(ma.ModelSchema):
    ...
    employee = ma.Nested(EmployeeSchema, only=('name','address','email', 'phone'))
...
class JobList(Resource):
    ...
    def get(self):
        ...
        if jobcode is not None:
            ...
            query = Job.query.filter_by(entity_id=request.args.get('entity'))
        elif request.args.get('employee'):
            query = Job.query.filter_by(employee_id=request.args.get('employee'))
        elif request.args.get('request'):
            query = Job.query.filter_by(request_id=request.args.get('request'))
        else:
            query = Job.query
    ...
    def post(self):
        ...
        try:
            ...
            db.session.commit()
            return (
                {"msg": "job created", "job": schema.dump(job).data},
                201,
            )
